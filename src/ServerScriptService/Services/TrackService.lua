...
function TrackService:SetupLighting()
    ...
    local cc = Lighting:FindFirstChild("RushColor") or Instance.new("ColorCorrectionEffect")
    cc.Name = "RushColor"
    cc.Saturation = .18
    cc.Contrast = .05
    cc.Brightness = .03
    cc.TintColor = Color3.fromRGB(255, 237, 224)
    cc.Parent = Lighting

    local bloom = Lighting:FindFirstChild("RushBloom") or Instance.new("BloomEffect")
    bloom.Name = "RushBloom"
    bloom.Intensity = .18
    bloom.Size = 20
    bloom.Threshold = 1.3
...
function TrackService:Build()
    if workspace:FindFirstChild("RunnerWorld") then
        workspace.RunnerWorld:Destroy()
    end
    ...
    spawn.Position = Vector3.new(0,1,Config.START_Z)
...
function TrackService:GenerateSegment(index)
    ...
    for _,side in ipairs({-1,1}) do
        ...
        local b = part(folder, "Building", Vector3.new(width,height,44), Vector3.new(baseX,height/2,z), color, Enum.Material.SmoothPlastic, true)
        ...
        if index % 2 == 0 then
            lamp(folder, side*23, z-18)
        end
        if index % 3 == 0 then
            tree(folder, side*23, z+15)
        end
    end
